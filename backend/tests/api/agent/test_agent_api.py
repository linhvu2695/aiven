...
async def test_search_agents_api():
    mock_response = SearchAgentsResponse(agents=[
        AgentInfo(id="1", name="Agent 1", description="Desc 1", model=LLMModel.GPT_3_5_TURBO, persona="helpful", tone="friendly"),
        AgentInfo(id="2", name="Agent 2", description="Desc 2", model=LLMModel.GPT_4, persona="expert", tone="serious"),
    ])
    ...
    with patch("app.services.agent.agent_service.AgentService.search_agents", new=AsyncMock(return_value=mock_response)):
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as ac:
            response = await ac.post("/agents/search")
